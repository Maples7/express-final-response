...
const defaultStatuses = require('./status.json');
...
module.exports = function genFinalResponse({
  ...
  const allStatuses = _.concat(defaultStatuses, customStatuses);

  // gen status map
  const STATUSES = {};
  _.forEach(allStatuses, (item) => {
    STATUSES[item.status] = item;
  });
  ...
  function handleNormalRequest(result, req, res) {
    ...
    const finalResp = STATUSES[result.status] || STATUSES['success'];
